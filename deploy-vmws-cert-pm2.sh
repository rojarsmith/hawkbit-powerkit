...
echo "$SUDO_PASS" | sudo tee /opt/hawkbit/config/ecosystem.config.js <<'EOF'
module.exports = {
  apps: [
    {
      ...
      args: [
        "-jar",
        "/opt/hawkbit/hawkbit-update-server.jar",
        "--spring.config.location=/opt/hawkbit/config/",
        "--spring.profiles.active=prod",
        "--spring.config.name=application,application-mysql,application-rabbitmq"
      ],
      exec_interpreter: "",
      exec_mode: "fork",
      error_file: "/opt/hawkbit/log/hawkbit-error.log",
      out_file: "/opt/hawkbit/log/hawkbit-out.log"
    },
    {
      name: "hawkbit-ui",
      script: "java",
      ...
      args: [
        "-jar","hawkbit-ui.jar",
        "--spring.config.additional-location=/opt/hawkbit/config/",
        "--spring.profiles.active=prod",
        "--spring.config.name=application-ui-ext"
      ],
      exec_interpreter: "",
      exec_mode: "fork",
      error_file: "/opt/hawkbit/log/hawkbit-ui-error.log",
      out_file: "/opt/hawkbit/log/hawkbit-ui-out.log"
    }
  ]
}
EOF
...
sudo tee /opt/hawkbit/config/application-ui-ext-prod.properties <<'EOF'
...
sudo chown -R srv:srv /opt/hawkbit
sudo chmod -R u+rwX /opt/hawkbit

cd /opt/hawkbit/config
pm2 delete hawkbit-update-server || true
pm2 delete hawkbit-ui || true
pm2 start ecosystem.config.js

# Enable PM2 startup on boot for root
pm2 startup systemd -u srv --hp /srv
pm2 save
